...
async function doLogin(){
    ...
    /* 🔑 لا نمنع الدخول لغياب المستخدم من _users:
       على هاتف جديد يكون السجل المحلي فارغاً. المرجع الوحيد الموثوق هو مصادقة Firebase.
       إن نجحت كلمة المرور فالمستخدم صحيح، ونُنشئ سجله محلياً بعد الدخول. */
    const user=users[uname]||_usersCache[uname]||null;

    /* التحقّق من كلمة المرور عبر مصادقة Firebase حصراً (لا بصمة مخزّنة تُكسَر) */
    const _ok=await _fbSignInEmail(uname,pw,false);
    if(btn){btn.disabled=false;btn.textContent=origTxt;}
    if(!_ok){
        /* فشل الدخول: إمّا كلمة مرور خاطئة أو مستخدم غير موجود في Firebase */
        return _showLoginErr(user?'كلمة المرور خاطئة':'اسم المستخدم أو كلمة المرور خاطئة');
    }
    /* نظّف أي بصمة قديمة متبقّية + اضمن وجود سجل المستخدم على هذا الجهاز */
    try{ if(!user||user.pwHash!==undefined){_usersCache[uname]={isAdmin:true};window._pendingUserWrite=uname;} }catch(e){}

    /* 👤 اكتشف إن كان موظفاً: ابحث عن سجله في كل مساحات المحلات المعروفة.
       الموظف يُسجَّل في goldpro/{owner}/_users/{uname} بدور employee + owner. */
    try{
        if(!_usersCache[uname]||!_usersCache[uname].role){
            /* جرّب مساحة مالك السريال أولاً (الأكثر شيوعاً) */
            const _own=(window._snOwner||'').toLowerCase();
            if(_own){
                const snap=await _db.ref(`goldpro/${_own}/_users/${uname}`).once('value');
                const rec=snap.val();
    ...
    /* 👤 تحديد الدور والصلاحيات:
       - employee: مساحة مالكه · صلاحيات مقيّدة
       - admin ثانٍ: مساحة مالكه · صلاحيات كاملة (شريك) */
    ...
    _baseRef=_db.ref('goldpro/'+_dataSpace+'/data');
    window._cfgRef=_db.ref('goldpro/'+_dataSpace+'/cfg');
    if(window._applyRolePerms)window._applyRolePerms();
    if(window._attachUserCfg)window._attachUserCfg();
    localStorage.setItem('gp12_auth','1');localStorage.setItem('gp12_user',uname);
    /* 👤 احفظ الدور ومساحة المالك كي تُستعاد الجلسة للموظف/الأدمين الثاني بلا إعادة دخول */
    try{
        const _r=window._userRole||'admin';
        if(_r==='employee'){
            ...
            localStorage.removeItem('gp12_role');localStorage.removeItem('gp12_owner');
        }
    }catch(e){}
...
function doLogout(){
    ...
    _encKey='';
    /* 🧹 نظّف صلاحيات الموظف كي لا تبقى مفعّلة على شاشة الدخول */
    try{ document.body.classList.remove('role-employee'); window._userRole='admin'; window._userPerms=null; window._dataSpace=null; }catch(e){}
    ...
    ['gp12_role','gp12_owner'].forEach(k=>{try{localStorage.removeItem(k);sessionStorage.removeItem(k);}catch(e){}});
...
async function addUser(){
    ...
    if(!uname||uname.length<3)return toast('اسم المستخدم قصير (3 أحرف على الأقل)','error');
    ...
    try{
        /* أنشئ حساب مصادقة للموظف — نحفظ جلسة المالك ونعيدها بعد الإنشاء */
        const _ownerEmail=firebase.auth().currentUser?.email;
        try{ await firebase.auth().createUserWithEmailAndPassword(_authEmail(uname),_fbPw(pw)); }
        catch(e){
            if(e&&e.code==='auth/email-already-in-use'){ /* موجود — نكمل لتسجيله كموظف */ }
            else throw e;
        }
        ...
        const code=(e&&e.code)||'';
        if(code==='auth/operation-not-allowed')return toast('فعّل Email/Password في Firebase Console','error');
        toast('تعذّرت الإضافة: '+(code||'خطأ'),'error');
    }
}

async function deleteUser(uname){
    if(!confirm(`حذف الموظف «${uname}»؟\nلن يعود قادراً على الدخول.`))return;
    try{
        await _db.ref(`${_usersPathFor()}/${uname}`).remove();
        delete _usersCache[uname];
        toast('✅ حُذف الموظف','success');
        renderUsersList();
    }catch(e){ toast('تعذّر الحذف','error'); }
}
...
async function _checkAuth(){
    ...
    const savedRole=localStorage.getItem('gp12_role')||sessionStorage.getItem('gp12_role')||'';
    ...
    if((localStorage.getItem('gp12_auth')==='1'||sessionStorage.getItem('gp12_auth')==='1')&&savedUser){
        ...
        const _savedOwner=localStorage.getItem('gp12_owner')||sessionStorage.getItem('gp12_owner')||'';
        const _space=(_isLinked&&_savedOwner)?_savedOwner:savedUser;
        window._dataSpace=_space;
        _LSKEY='gp12_'+(_SITE?_SITE+'_':'')+_space;
        _LSDRAFT='gp12_draft_'+(_SITE?_SITE+'_':'')+_space;
        _baseRef=_db.ref('goldpro/'+_space+'/data');
        window._cfgRef=_db.ref('goldpro/'+_space+'/cfg');
        if(window._attachUserCfg)window._attachUserCfg();
        const ud=document.getElementById('currentUserDisplay');if(ud)ud.textContent=savedUser;
        /* طبّق الصلاحيات كاملةً قبل إزالة شاشة الدخول (لا ومضة) */
        if(window._applyRolePerms)window._applyRolePerms();
...
window._applySerialLock=()=>{
    ...
    if(owner){
        /* مطالَب به → المالك مقترَح، لكن الحقل مفتوح ليدخل الموظف اسمه */
        if(setupP)setupP.style.display='none';
        if(mainP)mainP.style.display='block';
        if(uEl){
            uEl.value=owner; uEl.readOnly=false;
            uEl.style.opacity='1'; uEl.style.cursor='text';
        }
        if(hint){
            hint.innerHTML=`🏪 <strong>${window._snName||owner}</strong> — أدخل كلمة المرور<br><span style="font-size:.62rem;color:var(--t3);font-weight:600">موظف؟ امسح الاسم واكتب اسمك وكلمة سرك</span>`;
