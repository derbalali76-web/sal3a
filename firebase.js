    ...
    function applyBars(){
        ...
        if(d.barsAdd&&d.barsAdd.length){
            d.barsAdd.forEach(bar=>{
                ...
                if(bar.empOwner)nb.empOwner=bar.empOwner;   /* 👤 مخزون الموظف */
    ...
    switch(evt.type){
        ...
        case 'OPENING':{
            ...
            (d.debtRows||[]).forEach(r=>{
                ...
                stUpdDebt(r.c,r.type,sign*r.amt,r.empOwner||null);
            ...
            if(d.gv===2){
                ...
                const _deduct=(name,weight,karat)=>{
                    ...
                    for(let i=st.goodsStock.length-1;i>=0&&rem>0.0005;i--){
                        ...
                        /* 🏷️ حارس العيار: إن حُدّد عيار، اخصم من نفس العيار فقط */
                        if(karat!=null&&Number(g.k)!==Number(karat))continue;
                ...
                if(d.isBuy){
                    ...
                    if(rot){
                        ...
                        stUpdDebt(d.c,'دولار',Number(rot.eq)||0,d.empOwner||null);
                        if(Number(rot.fv))stUpdDebt(d.c,'دينار',Number(rot.fv),d.empOwner||null);
                    }
                    ...
                    stUpdDebt(d.c,'دولار',eq,d.empOwner||null);
                    if(fee)stUpdDebt(d.c,'دينار',fee,d.empOwner||null);
                    /* 🔥 كاصي حي: الزبون أعطاني وزناً خاماً — يُسجّل مؤقتاً كوزن 705 (ينقص دينه)
                       ويدخل مخزون الكاصي الحي بانتظار التذويب */
                    if(d.liveKassi&&(d.liveKassi.w||0)>0){
                        stUpdDebt(d.c,'دولار',-(Number(d.liveKassi.w)||0)); /* دفع مؤقت بوزن 705 */
                        st.liveKassi.push({
                            id:d.liveKassi.id,w:Number(d.liveKassi.w)||0,c:d.c,
                            did:evt.id||d.liveKassi.did||'',dt:d.liveKassi.dt||'',
                            prov:Number(d.liveKassi.w)||0,  /* المسجّل مؤقتاً */
                            melted:false
                        });
                    }
        ...
        case 'MELT_KASSI':{
            /* 🔥 تذويب كاصي حي: نصحّح الحساب من الوزن المؤقت إلى الوزن الحقيقي (705) */
            const lk=st.liveKassi.find(x=>x.id===d.id);
            if(lk&&!lk.melted){
                const real705=Math.round((Number(d.meltW)||0)*(Number(d.meltK)||0)/705*100)/100;
                const prov=Number(lk.prov||lk.w||0);
                /* الدفع سُجّل مؤقتاً −prov. الحقيقي −real705.
                   التصحيح = prov − real705 (يُضاف للدين فيصبح الدفع = الحقيقي) */
                const correction=Math.round((prov-real705)*100)/100;
                const diff=Math.round((real705-prov)*100)/100; /* للعرض: الحقيقي − المؤقت */
                stUpdDebt(lk.c,'دولار',correction);
                /* السبيكة الحقيقية تدخل مخزون الذهب حسب عيارها */
                const k=Number(d.meltK)||705;
                const bar={id:(evt.id||'')+'_melt',w:Number(d.meltW)||0,k,desc:'كاصي حي مذوّب · '+(lk.c||''),src:lk.c||'',origin:'كاصي حي',dt:d.dt||'',_ts:evt.ts||0};
                if(k>=999)st.g24.push(bar); else st.g730.push(bar);
                /* علّم القطعة مذوّبة */
                lk.melted=true; lk.meltW=Number(d.meltW)||0; lk.meltK=k; lk.real705=real705; lk.diff=diff; lk.meltDt=d.dt||'';
            }
            break;
        }
        ...
        case 'ADMIN_BUY_EMP_BAR':{
            ...
            if(bar&&bar.empOwner===d.emp){
                delete bar.empOwner;   /* صارت ملك الأدمين */
                bar.src='شراء من '+d.emp;
                const total=Number(d.total)||0;
                if(d.paid){
                    /* خالص: المبلغ يدخل صوارد كوفر الموظف (بالدينار) */
        ...
        case 'EMP_TAKE':{
            /* 🖐️ الموظف يأخذ مبلغاً من الأدمين:
               - ينقص سيولة المحل (الأدمين أعطاه)
               - يزيد صوارد كوفر الموظف
               - يُسجّل ديناً: الموظف يدين للحانوت (أخذ منه) */
            const emp=d.emp||'';
            const a=Number(d.a)||0;
            st.B.دينار-=a;                                  /* ينقص من سيولتك */
            st.empCoffer[emp]=(st.empCoffer[emp]||0)+a;      /* يزيد كوفره */
            stUpdDebt(emp,'دينار',a,emp);                    /* موجب = الموظف يدين للحانوت · موسوم به */
            break;
        }
        ...
        case 'INVOICE_BUY':{
            ...
            if(remB>0.001)stUpdDebt(d.c,'دينار',-remB,d.empOwner||null);
            else if(remB<-0.001)stUpdDebt(d.c,'دينار',Math.abs(remB),d.empOwner||null);
        ...
        case 'INVOICE_SELL':{
            ...
            if(remS>0.001)stUpdDebt(d.c,'دينار',remS,d.empOwner||null);
            /* 🏷️ بيع سبائك 705 يعكس أثر الشراء على «ذهب البيع»: يزيد cashiBuyW (يُنقص «تبيعه») */
            if(Array.isArray(d.barsRemove)&&d.barsRemove.length){
                let soldEq705=0;
                /* نجمع من بنود الفاتورة (705 فقط) */
                (disp.invoice&&disp.invoice.items||[]).forEach(it=>{
                    if(it.is1000||Number(it.k)>=999)return; /* 24 لا يدخل الكاصي */
                    soldEq705+=(Number(it.w)||0)*((Number(it.k)||730)/705);
                });
                if(soldEq705>0){
                    st.cashiBuyW+=soldEq705;
                    st.cashiBuyDin+=(d.tp||0);
                }
            }
            if(disp.invoice)st.invoices.unshift(disp.invoice);
            break;
        ...
        case 'SETTLE_CASH':{
            ...
            if(Math.abs(rem)>0.001)stUpdDebt(d.c,d.type,dir*rem,d.empOwner||null);
            break;
        }
        ...
        case 'SETTLE_GSM':{
            ...
            if(d.freeBuy){
                ...
                stUpdDebt(c,type,d.w,d.empOwner||null);
                stUpdDebt(c,'دينار',cashTotal,d.empOwner||null);
            }else{
                if(!isBuy)applyBars();
                stClearDebt(c,type);
        ...
        case 'XFER':{
            ...
            stUpdDebt(d.from, d.srcType, -d.srcDelta, d.empOwner||null);   // إنقاص من حساب المصدر
            stUpdDebt(d.to,   d.dstType,  d.dstDelta, d.empOwner||null);   // إضافة لحساب الهدف (بنفس الاتجاه)
...
function _reproject(){
    ...
    const st={
        ...
        liveKassi:[],  /* 🔥 مخزون الكاصي الحي (خام قبل التذويب) */
        empCoffer:{},  /* 👤 صوارد كوفر كل موظف */
    ...
    window._liveKassi=st.liveKassi||[];  /* 🔥 مخزون الكاصي الحي */
    window._empCoffer=st.empCoffer||{};  /* 👤 صوارد كوفر الموظفين */
    ...
    if(window._userRole==='employee'&&typeof _renderEmpHome==='function')try{_renderEmpHome();}catch(e){}
...
function save(){
    ...
    /* 🌙 الوضع الليلي تفضيل شخصي لكل جهاز — لا يُزامَن كي لا يغيّره الموظف على الأدمين */
    try{_baseRef.child('settings').set(_withOwner({goldPrice,dollarRate,dollarSellRate,dollarBuyRate,dubaiCalc:_dc,tarbah:_tb,_ts:firebase.database.ServerValue.TIMESTAMP})).catch(_fbErr);}catch(e){}
}
...
function _fbInitialLoad(){
    ...
    _baseRef.child('settings').once('value',s=>{
        ...
        if(cfg){
            ...
            /* 🌙 لا نقرأ الوضع الليلي من السحابة — يبقى تفضيل الجهاز */
...
window._trashList=()=>{
    ...
    /* 👤 كل مستخدم يرى محذوفاته فقط: الأدمين عناصره · الموظف عناصره */
    ...
    return _allEvents.filter(e=>e.type==='VOID'&&live.has(e.id)).map(v=>{
        ...
        let desc='عملية', _owner=null;
        if(t){
            const dsp=t.display||{}, dd=t.data||{};
            const inv=dsp.invoice||dsp.dollInvoice||dsp.rafInvoice||dsp.dubaiInvoice;
            /* مالك العنصر المحذوف (موظف أم أدمين) */
            _owner=(dsp.op&&dsp.op.empOwner)||(inv&&inv.empOwner)||dd.empOwner||null;
        ...
        return {vid:v.id,ts:v.ts||0,desc,targetType:t?t.type:'?',owner:_owner};
    })
    .filter(it=> _isEmp ? (it.owner===_me) : (!it.owner))
    .sort((a,b)=>b.ts-a.ts);
};
window._restoreVoid=(vid)=>{
    /* 🔒 حارس الملكية: كلٌّ يسترجع محذوفاته فقط */
    try{
        const items=window._trashList?window._trashList():[];
        if(items.length&&!items.some(it=>it.vid===vid)){
            if(typeof toast==='function')toast('🚫 لا يمكنك استرجاع عنصر ليس لك','error');
            return;
        }
    }catch(e){}
    emitEvent('VOID',{voids:vid},null);
};
