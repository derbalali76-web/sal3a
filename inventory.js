...
function renderInvModal(){
    const allBars=invType==='24'?g24:g730;
    /* 👤 الأدمين يرى سبائكه فقط · الموظف سبائكه · سبائك الموظفين لها بطاقة لاقوبال */
    const _me=window._currentUser, _isEmp=window._userRole==='employee';
    const bars=allBars.filter(b=> _isEmp ? (b.empOwner===_me) : (!b.empOwner));
    ...
    c.innerHTML=[...bars].reverse().map(b=>`
        <div class="bar-item">
            <div class="bar-info">
                ...
                ${b.src?`<span style="display:inline-block;font-size:.6rem;background:rgba(13,148,136,.14);color:#0d9488;padding:.05rem .35rem;border-radius:.5rem;margin-right:.25rem;font-weight:800">👤 ${b.src}${b.origin?' · '+b.origin:''}</span>`:''}
                <small>${b.src?'':(b.desc||'')}${b.dt?' | '+b.dt:''}</small>
            </div>
