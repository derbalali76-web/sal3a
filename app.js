...
function openSettings(){
    ...
    /* 🔒 عزل الموظف: أخفِ إعدادات الأدمين مباشرةً (لا نعتمد على CSS وحده) */
    const _isEmp=window._userRole==='employee';
...
const _LIQ_USED_KEY_BASE='gp12_liq_set';
/* مفتاح منفصل لكل موظف كي تكون أرصدته الافتتاحية لمرة واحدة مستقلة عن الأدمين */
function _liqUsedKey(){ return window._userRole==='employee'?(_LIQ_USED_KEY_BASE+'_emp_'+(window._currentUser||'')):_LIQ_USED_KEY_BASE; }
...
window.confirmLiqEdit=()=>{
    ...
    document.querySelectorAll('#liqDebtRows tr[id^="liqRow_"]').forEach(tr=>{
        ...
        if(din)debtRows.push({c,type:'دينار',amt:din,dir:'لنا',kind,empOwner:_emp});   /* الإشارة في القيمة نفسها */
        if(gd) debtRows.push({c,type:'دولار',amt:gd, dir:'لنا',kind,empOwner:_emp});
    });
...
/* ═══════════ BALANCE ═══════════ */
function syncBal(){
    /* 👤 رصيد الأدمين = سبائكه فقط (سبائك الموظفين لها بطاقة لاقوبال منفصلة) */
    const adminBars24=g24.filter(b=>!b.empOwner);
    const adminBars730=g730.filter(b=>!b.empOwner);
    B['ذهب 24'] =adminBars24.reduce((s,b)=>s+(b.w||0),0);
    /* المخزون 730 = مجموع (وزن × عيار ÷ 730) لكل قطعة (بلا سبائك الموظفين) */
    B['ذهب 730']=adminBars730.reduce((s,b)=>s+(b.w||0)*((b.k||730)/730),0);
}
function _netBuckets(){
    /* 👤 حسابات الأدمين المالية تستثني ديون الموظفين (لها لاقوبال منفصل) */
    const _ad=debts.filter(d=>!d.empOwner);
    const d_din =_ad.reduce((s,d)=>d.type==='دينار'  ?s+(d.a||0):s,0);
    const d_dol =_ad.reduce((s,d)=>d.type==='دولار'  ?s+(d.a||0):s,0);  /* ديون السلعة (غ 705) */
    const d_730 =_ad.reduce((s,d)=>d.type==='ذهب 730'?s+(d.a||0):s,0);  /* بمكافئ 730 */
    const d_24  =_ad.reduce((s,d)=>d.type==='ذهب 24' ?s+(d.a||0):s,0);  /* بالغرام الخام 24 */
...
function upd(){
    ...
    /* 🔥 بطاقة الكاصي الحي: تظهر فقط إن كان فيه قطع غير مذوّبة */
    try{
        const lk=(window._liveKassi||[]).filter(x=>!x.melted);
        const card=document.getElementById('liveKassiCard');
        if(card){
            const totW=lk.reduce((s,x)=>s+(Number(x.w)||0),0);
            if(lk.length){ card.style.display=''; const b=document.getElementById('liveKassiBal'); if(b)b.innerHTML=fmt(totW,2)+'<small> g</small>'; }
...
const _GTAB_COLORS={secRotor:'#d4af37',secCash:'#0369a1',secKass:'#0d9488',secCashi:'#b45309',secCashiFee:'#7c3aed',secLiveKassi:'#ea580c'};
...
window._resetGoodsSecs=()=>{
    ['secRotor','secCash','secKass','secCashi','secCashiFee','secLiveKassi'].forEach(id=>{const e=document.getElementById(id);if(e)e.style.display='none';});
...
window.openGoodsFor=(name)=>{
    ...
    ov.innerHTML=`<div class="modal" style="max-width:420px">
        ...
        <div style="display:flex;flex-direction:column;gap:.45rem">
            ${window._userRole!=='employee'?`<button onclick="document.getElementById('custCardOverlay').classList.remove('active');openDollar('${actType}','${name.replace(/'/g,"\\'")}')"
            ...
            <div style="display:flex;gap:.45rem">
                ...
                <button onclick="document.getElementById('custCardOverlay').classList.remove('active');viewCustomerLogDirect('${name.replace(/'/g,"\\'")}')"
                    style="flex:1;padding:.65rem;border:1.5px solid #7c3aed;border-radius:12px;background:transparent;color:#7c3aed;font-family:Tajawal,sans-serif;font-weight:900;font-size:.82rem;cursor:pointer">عرض السجل</button>
            </div>
...
window.openDollar=(t,prefillName)=>{
    ...
    /* 🔥 كاصي حي: يظهر عند البيع فقط (الزبون يعطيني خاماً) */
...
/* المتاح من سلعة بعيار محدّد (حارس العيار) */
function _stockAvailK(name,karat){ return goodsStock.filter(g=>g.n===name&&Number(g.k)===Number(karat)).reduce((s,g)=>s+(g.w||0),0); }
...
window.saveDollar=()=>{
    ...
    /* 🔥 اقرأ الكاصي الحي أولاً (بيع فقط) */
    const _lkW=!isBuy?readNum('liveKassiW'):0;
    const loose=_readGoodsRows(true);
    /* لا تمنع لو الصف فارغ والكاصي الحي وحده موجود */
    const hasRealRow=loose.some(it=>(it.w||0)>0);
    const noName=loose.find(it=>!it.n&&(it.w||0)>0);
    if(noName)return toast(`⚠️ اختر اسم السلعة في السطر ${noName._row} — أضف الأسماء من ⚙ الإعدادات أو اختر «＋ اسم جديد…» من القائمة`,'error');
    const items=loose.filter(it=>it.n&&(it.w||0)>0);
    ...
    const cashiCash=_readCashiCash();
    const cashiFee=_readCashiFee();
    if(!items.length&&!rot&&!cash&&!kass.eq&&!cashiCash&&!cashiFee.eq&&!(_lkW>0))return toast('أدخل سطراً مكتملاً واحداً على الأقل','error');
    ...
    const fee=items.reduce((s,it)=>s+it.fv,0);
    /* 🚫 حارس الاسم والعيار: البيع يُخصم من نفس الاسم ونفس العيار فقط */
    if(!isBuy){
        const need={};
        items.forEach(it=>{ const key=it.n+'|'+it.k; need[key]=(need[key]||0)+it.w; });
        for(const key in need){
            const [nm,kk]=key.split('|');
            const avail=_stockAvailK(nm,Number(kk));
            if(need[key]>avail+0.0005){
                const availAny=_stockAvail(nm);
                if(availAny>0.0005){
                    /* يوجد نفس الاسم لكن بعيار مختلف */
                    const karats=[...new Set(goodsStock.filter(g=>g.n===nm&&g.w>0.0005).map(g=>g.k))].join('، ');
                    return toast(`⚠️ «${nm}» عيار ${kk} غير متوفّر — المتاح بعيارات: ${karats}. المطلوب ${fmt(need[key],2)}غ عيار ${kk}`,'error');
                }
                return toast(`⚠️ مخزون «${nm}» غير كافٍ — متاح ${fmt(avail,2)}غ عيار ${kk} والمطلوب ${fmt(need[key],2)}غ`,'error');
            }
        }
    }
    const did='DOLL-'+uid();
    ...
    const _gnote=(document.getElementById('goodsNote')?.value||'').trim();
    const _gEmp=(window._userRole==='employee'?_currentUser:null);
    emitEvent('DOLLAR',
        {gv:2,c:cust,isBuy,items,equiv,fee,a:equiv,r:0,rot:rot||null,cash:cash||0,kass:kass.eq>0?kass:null,cashiCash:cashiCash||null,cashiFee:cashiFee.eq>0?cashiFee:null,liveKassi:liveKassi||null,note:_gnote||null,empOwner:_gEmp},
        {dollInvoice:_di,op:{c:cust,t:_opName,m:_opM,a:_opA,fee,_ts:Date.now(),dt:nowStr,did,gItems:items.length,rotW:rot?rot.w:0,note:_gnote||undefined,empOwner:(window._userRole==='employee'?_currentUser:undefined)}}
    );
...
/* 🔥 مخزون الكاصي الحي — عرض القطع الخام + زر تذويب */
window.openLiveKassi=()=>{
    let ov=document.getElementById('liveKassiOverlay');
    if(!ov){
        ov=document.createElement('div');ov.id='liveKassiOverlay';ov.className='modal-overlay';
        document.body.appendChild(ov);
...
function _renderLiveKassi(){
    const ov=document.getElementById('liveKassiOverlay'); if(!ov)return;
    const lk=(window._liveKassi||[]).filter(x=>!x.melted);
    const totW=lk.reduce((s,x)=>s+(Number(x.w)||0),0);
    ov.innerHTML=`<div class="modal" style="max-width:460px">
        <div class="modal-handle"></div>
        <h3 style="text-align:center;color:#ea580c">🔥 مخزون الكاصي الحي</h3>
        <div style="text-align:center;font-size:.7rem;color:var(--t3);margin-bottom:.5rem">الإجمالي الخام: <b style="color:#ea580c">${fmt(totW,2)} غ</b></div>
        <div style="max-height:56vh;overflow-y:auto;display:flex;flex-direction:column;gap:.4rem">
            ${lk.length?lk.map(x=>`
                <div style="display:flex;align-items:center;justify-content:space-between;padding:.6rem .7rem;border:1.5px solid rgba(234,88,12,.3);border-radius:12px;background:rgba(234,88,12,.05);gap:.5rem">
                    <div style="flex:1;min-width:0">
                        <div style="font-weight:900;font-size:1rem;color:#ea580c">${fmt(x.w,2)} غ</div>
                        <div style="font-size:.62rem;color:var(--t3)">👤 ${x.c||'—'} · ${x.dt||''}</div>
                    </div>
                    <button onclick="openMeltKassi('${x.id}')" style="padding:.5rem 1rem;border:none;border-radius:10px;background:linear-gradient(135deg,#ea580c,#c2410c);color:#fff;font-family:Tajawal,sans-serif;font-weight:900;font-size:.8rem;cursor:pointer;white-space:nowrap">🔥 تذويب</button>
                </div>`).join(''):'<div style="text-align:center;color:var(--t3);padding:2rem">لا يوجد كاصي حي بانتظار التذويب</div>'}
        </div>
        <button onclick="document.getElementById('liveKassiOverlay').classList.remove('active')"
            style="width:100%;margin-top:.6rem;padding:.6rem;border:none;border-radius:10px;background:rgba(120,120,120,.12);color:var(--t2);font-family:Tajawal,sans-serif;font-weight:800;cursor:pointer">إغلاق</button>
    </div>`;
}
window.openMeltKassi=(id)=>{
    const lk=(window._liveKassi||[]).find(x=>x.id===id&&!x.melted); if(!lk)return;
    let ov=document.getElementById('meltKassiOverlay');
    if(!ov){
        ov=document.createElement('div');ov.id='meltKassiOverlay';ov.className='modal-overlay';
        document.body.appendChild(ov);
        ov.addEventListener('click',e=>{if(e.target===ov)ov.classList.remove('active');});
    }
    ov.innerHTML=`<div class="modal" style="max-width:380px">
        <div class="modal-handle"></div>
        <h3 style="text-align:center;color:#ea580c">🔥 تذويب كاصي حي</h3>
        <div style="text-align:center;font-size:.72rem;color:var(--t3);margin-bottom:.6rem">القطعة: <b>${fmt(lk.w,2)} غ</b> · 👤 ${lk.c||'—'}<br>سُجّلت مؤقتاً في دينه كوزن 705</div>
        <input type="text" inputmode="decimal" id="meltW" placeholder="⚖️ الوزن الجديد بعد التذويب (غ)" dir="ltr"
            style="width:100%;padding:.6rem .7rem;border-radius:10px;border:1.5px solid #ea580c;background:var(--card2);color:var(--t);font-family:Tajawal,sans-serif;font-size:.9rem;font-weight:800;text-align:right;margin-bottom:.5rem;box-sizing:border-box"
            oninput="liveNum(this);_meltPreview('${id}')">
        <input type="text" inputmode="decimal" id="meltK" placeholder="🏷️ العيار (مثلاً 680)" dir="ltr"
            style="width:100%;padding:.6rem .7rem;border-radius:10px;border:1.5px solid #ea580c;background:var(--card2);color:var(--t);font-family:Tajawal,sans-serif;font-size:.9rem;font-weight:800;text-align:right;margin-bottom:.5rem;box-sizing:border-box"
            oninput="liveNum(this);_meltPreview('${id}')">
        <div id="meltPreview" style="padding:.6rem;border-radius:10px;background:var(--card2);font-size:.75rem;text-align:center;margin-bottom:.5rem;min-height:2.5rem;display:flex;align-items:center;justify-content:center;color:var(--t3)">أدخل الوزن والعيار</div>
        <div style="display:flex;gap:.5rem">
            <button onclick="document.getElementById('meltKassiOverlay').classList.remove('active')"
            ...
            <button onclick="saveMelt('${id}')"
                style="flex:2;padding:.65rem;border:none;border-radius:12px;background:linear-gradient(135deg,#ea580c,#c2410c);color:#fff;font-family:Tajawal,sans-serif;font-weight:900;cursor:pointer">💾 حفظ التذويب</button>
        </div>
    </div>`;
    ov.classList.add('active');
    setTimeout(()=>{const e=document.getElementById('meltW');if(e)e.focus();},300);
};
window._meltPreview=(id)=>{
    const lk=(window._liveKassi||[]).find(x=>x.id===id); if(!lk)return;
    const w=readNum('meltW'), k=readNum('meltK');
    const el=document.getElementById('meltPreview'); if(!el)return;
    if(w>0&&k>0){
        const real705=Math.round(w*k/705*100)/100;
        const diff=Math.round((real705-(lk.prov||lk.w||0))*100)/100;
        el.innerHTML=`الوزن الحقيقي (705): <b style="color:#16a34a">${fmt(real705,2)} غ</b><br><span style="color:${diff<0?'#dc2626':'#16a34a'}">فرق العيار: ${diff>0?'+':''}${fmt(diff,2)} غ</span>`;
    }else el.textContent='أدخل الوزن والعيار';
};
window.saveMelt=(id)=>{
    const lk=(window._liveKassi||[]).find(x=>x.id===id&&!x.melted); if(!lk)return toast('القطعة غير موجودة','error');
    const w=readNum('meltW'), k=readNum('meltK');
    if(!w||w<=0)return toast('أدخل الوزن الجديد','error');
    if(!k||k<=0||k>1000)return toast('أدخل عياراً صحيحاً (1–1000)','error');
    const real705=Math.round(w*k/705*100)/100;
    const diff=Math.round((real705-(lk.prov||lk.w||0))*100)/100;
    const nowStr=new Date().toLocaleDateString('fr-FR',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'});
    const note=`دفع ${fmt(lk.w,2)}غ كاصي حي · بعد التذويب ${fmt(w,2)}غ عيار ${fmt(k,0)} = ${fmt(real705,2)}غ (705) · فرق العيار ${diff>0?'+':''}${fmt(diff,2)}غ`;
    emitEvent('MELT_KASSI',
        {id,meltW:w,meltK:k,dt:nowStr},
        {op:{c:lk.c,t:'تذويب كاصي حي',m:'دولار',a:diff,_ts:Date.now(),dt:nowStr,note}}
    );
    document.getElementById('meltKassiOverlay').classList.remove('active');
    setTimeout(()=>{ _renderLiveKassi(); toast('🔥 تم التذويب','success'); },300);
};
...
/* ═══════════ EXPENSE ═══════════ */
window.toggleExpCur=()=>{};
/* 👤 بطاقة دفع الموظف: مبلغ يدخل صوارد كوفره ويظهر عند الأدمين في سيولته */
...
window.openLaGobal=()=>{
    ...
    if(!ov){
        ov=document.createElement('div');ov.id='laGobalOverlay';ov.className='modal-overlay';
        document.body.appendChild(ov);
        ov.addEventListener('click',e=>{if(e.target===ov)ov.classList.remove('active');});
    }
    ov.innerHTML=`<div class="modal" style="max-width:460px">
        <div class="modal-handle"></div>
        <h3 style="text-align:center;color:#7c3aed">👤 لاقوبال — الموظفون</h3>
        ${list.length?list.map(emp=>`
            <div style="margin-bottom:.6rem;padding:.5rem;border:1.5px solid rgba(124,58,237,.25);border-radius:12px;background:rgba(124,58,237,.04)">
                <div style="font-weight:900;font-size:.9rem;color:#7c3aed;margin-bottom:.5rem;text-align:center">${emp}</div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:.4rem">
                    <button onclick="_laGobalDebts('${emp.replace(/'/g,"\\'")}')" style="padding:.6rem;border:1px solid var(--border);border-radius:10px;background:var(--card2);color:inherit;font-family:Tajawal,sans-serif;font-weight:800;font-size:.72rem;cursor:pointer"><i class="fas fa-book" style="color:#0369a1"></i><br>دفتر الديون</button>
                    <button onclick="_laGobalExp('${emp.replace(/'/g,"\\'")}')" style="padding:.6rem;border:1px solid var(--border);border-radius:10px;background:var(--card2);color:inherit;font-family:Tajawal,sans-serif;font-weight:800;font-size:.72rem;cursor:pointer"><i class="fas fa-receipt" style="color:#dc2626"></i><br>المصاريف</button>
                    <button onclick="_laGobalStock('${emp.replace(/'/g,"\\'")}')" style="padding:.6rem;border:1px solid var(--border);border-radius:10px;background:var(--card2);color:inherit;font-family:Tajawal,sans-serif;font-weight:800;font-size:.72rem;cursor:pointer"><i class="fas fa-crown" style="color:var(--g500)"></i><br>مخزون 705</button>
                    <button onclick="_laGobalDinar('${emp.replace(/'/g,"\\'")}')" style="padding:.6rem;border:1px solid var(--border);border-radius:10px;background:var(--card2);color:inherit;font-family:Tajawal,sans-serif;font-weight:800;font-size:.72rem;cursor:pointer"><i class="fas fa-wallet" style="color:#0d9488"></i><br>مخزون الدينار</button>
                </div>
                <button onclick="_laGobalTake('${emp.replace(/'/g,"\\'")}')" style="width:100%;margin-top:.4rem;padding:.6rem;border:none;border-radius:10px;background:linear-gradient(135deg,#ea580c,#c2410c);color:#fff;font-family:Tajawal,sans-serif;font-weight:900;font-size:.8rem;cursor:pointer"><i class="fas fa-hand-holding"></i> أخذ (يعطيه من رصيدك)</button>
                <button onclick="_laGobalLog('${emp.replace(/'/g,"\\'")}')" style="width:100%;margin-top:.35rem;padding:.55rem;border:1.5px solid #6366f1;border-radius:10px;background:transparent;color:#6366f1;font-family:Tajawal,sans-serif;font-weight:800;font-size:.76rem;cursor:pointer"><i class="fas fa-history"></i> سجل عملياته</button>
            </div>`).join(''):'<div style="text-align:center;color:var(--t3);padding:2rem">لا يوجد موظفون بعد</div>'}
        <button onclick="document.getElementById('laGobalOverlay').classList.remove('active')" style="width:100%;margin-top:.5rem;padding:.6rem;border:none;border-radius:10px;background:rgba(120,120,120,.12);color:var(--t2);font-family:Tajawal,sans-serif;font-weight:800;cursor:pointer">إغلاق</button>
    </div>`;
    ov.classList.add('active');
};
function _laGobalCard(title,color,bodyHtml){
    let ov=document.getElementById('laGobalSubOverlay');
    if(!ov){ ov=document.createElement('div');ov.id='laGobalSubOverlay';ov.className='modal-overlay';document.body.appendChild(ov);
        ov.addEventListener('click',e=>{if(e.target===ov)ov.classList.remove('active');}); }
    ov.innerHTML=`<div class="modal" style="max-width:440px">
        <div class="modal-handle"></div>
        <h3 style="text-align:center;color:${color}">${title}</h3>
        <div style="max-height:56vh;overflow-y:auto">${bodyHtml}</div>
        <button onclick="document.getElementById('laGobalSubOverlay').classList.remove('active')" style="width:100%;margin-top:.5rem;padding:.6rem;border:none;border-radius:10px;background:rgba(120,120,120,.12);color:var(--t2);font-family:Tajawal,sans-serif;font-weight:800;cursor:pointer">إغلاق</button>
    </div>`;
    ov.classList.add('active');
}
window._laGobalDebts=(emp)=>{
    const myDebts={};
    debts.forEach(d=>{ if(d.empOwner===emp){ myDebts[d.c]=myDebts[d.c]||{di:0,do:0}; if(d.type==='دينار')myDebts[d.c].di+=d.a; else if(d.type==='دولار')myDebts[d.c].do+=d.a; } });
    const entries=Object.entries(myDebts).filter(([,v])=>Math.abs(v.di)>0.001||Math.abs(v.do)>0.001);
    const html=entries.length?entries.map(([n,v])=>`
        <div style="display:flex;justify-content:space-between;align-items:center;padding:.55rem .6rem;border-bottom:1px solid var(--border)">
            <span style="font-weight:800;font-size:.82rem">${n}</span>
            <span style="text-align:left;font-size:.75rem">
                ${Math.abs(v.di)>0.001?`<span style="color:${v.di>0?'#dc2626':'#16a34a'};font-weight:800">${fmtDin(Math.abs(v.di))} دج</span>`:''}
                ${Math.abs(v.do)>0.001?`<br><span style="color:${v.do>0?'#dc2626':'#16a34a'};font-weight:800">${fmt(Math.abs(v.do),2)} غ</span>`:''}
            </span>
        </div>`).join(''):'<div style="text-align:center;color:var(--t3);padding:1.5rem">لا زبائن</div>';
    _laGobalCard('📖 دفتر ديون '+emp,'#0369a1',html);
};
window._laGobalExp=(emp)=>{
    const exps=ops.filter(o=>o.t==='مصاريف'&&o.empOwner===emp).sort((a,b)=>(b._ts||0)-(a._ts||0));
    const tot=exps.reduce((s,o)=>s+(o.a||0),0);
    const html=(exps.length?exps.map(o=>`
        <div style="display:flex;justify-content:space-between;padding:.5rem .6rem;border-bottom:1px solid var(--border)">
            <div><div style="font-weight:800;font-size:.8rem;color:#dc2626">−${fmtDin(o.a||0)} دج</div>
            <div style="font-size:.6rem;color:var(--t3)">${o.dt||''}${o.note?' · '+o.note:''}</div></div>
        </div>`).join(''):'<div style="text-align:center;color:var(--t3);padding:1.5rem">لا مصاريف</div>')
        +`<div style="margin-top:.5rem;padding:.5rem;background:var(--card2);border-radius:10px;text-align:center;font-weight:900">الإجمالي: <span style="color:#dc2626">${fmtDin(tot)} دج</span></div>`;
    _laGobalCard('💸 مصاريف '+emp,'#dc2626',html);
};
...
window._laGobalDinar=(emp)=>{
    ...
    const html=`<div style="text-align:center;padding:2rem">
        <div style="font-size:.75rem;color:var(--t3);margin-bottom:.5rem">صوارد كوفر ${emp}</div>
        <div style="font-size:1.6rem;font-weight:900;color:#0d9488">${fmtDin(coffer)} دج</div>
    </div>`;
    _laGobalCard('💰 مخزون الدينار — '+emp,'#0d9488',html);
};
...
window._laGobalLog=(emp)=>{
    ...
    const html=myOps.length?myOps.map(o=>{
        ...
        const dlines=(typeof opDetailLines==='function')?opDetailLines(o):[];
        const det=dlines.length?dlines.map(l=>`<br><span style="color:var(--t3);font-size:.58rem">${l}</span>`).join(''):'';
        return`<div style="padding:.5rem .6rem;border-bottom:1px solid var(--border)">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:.5rem">
                <div style="flex:1;min-width:0"><strong style="font-size:.76rem">${o.c||''}</strong>
                <br><small style="color:var(--t2);font-size:.62rem">${o.dt||''} · ${(window._realOpName?_realOpName(o):o.t)}</small>${det}</div>
                <span style="color:${out?'var(--rd)':'var(--gr)'};font-weight:900;font-size:.7rem;white-space:nowrap">${out?'−':'+'}${fmt(o.a||0,2)} ${unit}</span>
            </div>
        </div>`;}).join(''):'<div style="text-align:center;color:var(--t3);padding:1.5rem">لا عمليات</div>';
    _laGobalCard('📜 سجل عمليات '+emp,'#6366f1',html);
};
/* 🖐️ أخذ: الموظف يأخذ مبلغاً من رصيد الأدمين → ينقص سيولتك · يزيد كوفره · دين عندكما */
window._laGobalTake=(emp)=>{
    let ov=document.getElementById('takeEmpOverlay');
    if(!ov){
        ov=document.createElement('div');ov.id='takeEmpOverlay';ov.className='modal-overlay';document.body.appendChild(ov);
        ov.addEventListener('click',e=>{if(e.target===ov)ov.classList.remove('active');});
    }
    ov.innerHTML=`<div class="modal" style="max-width:380px">
        <div class="modal-handle"></div>
        <h3 style="text-align:center;color:#ea580c">🖐️ أخذ — ${emp}</h3>
        <div style="font-size:.68rem;color:var(--t3);text-align:center;margin-bottom:.6rem">المبلغ يُخصم من سيولتك ويدخل كوفر الموظف، ويُسجّل ديناً عندكما</div>
        <input type="text" inputmode="decimal" id="takeEmpAmount" placeholder="💰 المبلغ (دج)" dir="ltr"
            style="width:100%;padding:.7rem;border-radius:10px;border:1.5px solid #ea580c;background:var(--card2);color:var(--t);font-family:Tajawal,sans-serif;font-size:1rem;font-weight:800;text-align:right;margin-bottom:.5rem;box-sizing:border-box"
            oninput="liveNum(this)">
        <input type="text" id="takeEmpNote" placeholder="📝 ملاحظة (اختياري)"
        ...
        <div style="display:flex;gap:.5rem">
            <button onclick="document.getElementById('takeEmpOverlay').classList.remove('active')" style="flex:1;padding:.65rem;border:1.5px solid var(--border);border-radius:12px;background:transparent;color:var(--t2);font-family:Tajawal,sans-serif;font-weight:800;cursor:pointer">إلغاء</button>
            <button onclick="_saveLaGobalTake('${emp.replace(/'/g,"\\'")}')" style="flex:2;padding:.65rem;border:none;border-radius:12px;background:linear-gradient(135deg,#ea580c,#c2410c);color:#fff;font-family:Tajawal,sans-serif;font-weight:900;cursor:pointer">💾 تأكيد الأخذ</button>
        </div>
    </div>`;
    ov.classList.add('active');
    setTimeout(()=>{const e=document.getElementById('takeEmpAmount');if(e)e.focus();},300);
};
window._saveLaGobalTake=(emp)=>{
    const a=readNum('takeEmpAmount');
    const note=(document.getElementById('takeEmpNote').value||'').trim();
    if(!a||a<=0)return toast('أدخل المبلغ','error');
    if(B.دينار<a-0.001)return toast('⚠️ سيولتك غير كافية','error');
    const nowStr=new Date().toLocaleDateString('fr-FR',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'});
    emitEvent('EMP_TAKE',
        {emp,a,note},
        {op:{c:emp,t:'أخذ (موظف)',m:'دينار',a,_ts:Date.now(),dt:nowStr,note,empOwner:emp}}
    );
    document.getElementById('takeEmpOverlay').classList.remove('active');
    setTimeout(()=>{ toast(`🖐️ أخذ ${emp} ${fmtDin(a)} دج من رصيدك`,'success'); },200);
};
/* الأدمين يشتري سبيكة الموظف: تُسجّل كأن الموظف باعها للحانوت */
window._adminBuyEmpBar=(barId,emp)=>{
    const bar=(g730||[]).find(b=>b.id===barId&&b.empOwner===emp);
    if(!bar)return toast('السبيكة غير موجودة','error');
    const eq705=Math.round(bar.w*bar.k/705*1000)/1000;
    let ov=document.getElementById('buyEmpBarOverlay');
    if(!ov){
        ov=document.createElement('div');ov.id='buyEmpBarOverlay';ov.className='modal-overlay';document.body.appendChild(ov);
        ov.addEventListener('click',e=>{if(e.target===ov)ov.classList.remove('active');});
    }
    ov.innerHTML=`<div class="modal" style="max-width:390px">
        <div class="modal-handle"></div>
        <h3 style="text-align:center;color:#16a34a">🛒 شراء سبيكة من ${emp}</h3>
        <div style="text-align:center;font-size:.74rem;color:var(--t3);margin-bottom:.6rem">${fmt(bar.w,2)}غ · عيار ${fmt(bar.k,0)} = <b style="color:var(--g600)">${fmt(eq705,2)}غ (705)</b></div>
        <input type="text" inputmode="decimal" id="buyBarPrice" placeholder="💰 سعر الغرام (705) دج" dir="ltr"
            style="width:100%;padding:.65rem;border-radius:10px;border:1.5px solid #16a34a;background:var(--card2);color:var(--t);font-family:Tajawal,sans-serif;font-size:.95rem;font-weight:800;text-align:right;margin-bottom:.5rem;box-sizing:border-box"
            oninput="liveNum(this);_buyBarPreview(${eq705})">
        <div id="buyBarTotal" style="text-align:center;font-size:.8rem;color:var(--t2);margin-bottom:.5rem;min-height:1.2rem"></div>
        <div style="display:flex;gap:.5rem;margin-bottom:.6rem">
            <button type="button" id="buyBarPaid" onclick="window._buyBarPaidState=true;_buyBarSetPaid()" style="flex:1;padding:.6rem;border-radius:10px;border:1.5px solid #16a34a;background:#16a34a;color:#fff;font-family:Tajawal,sans-serif;font-weight:900;font-size:.8rem;cursor:pointer">خالص (نقداً)</button>
            <button type="button" id="buyBarUnpaid" onclick="window._buyBarPaidState=false;_buyBarSetPaid()" style="flex:1;padding:.6rem;border-radius:10px;border:1.5px solid #dc2626;background:transparent;color:#dc2626;font-family:Tajawal,sans-serif;font-weight:900;font-size:.8rem;cursor:pointer">غير خالص (دين)</button>
        </div>
        <div style="display:flex;gap:.5rem">
            <button onclick="document.getElementById('buyEmpBarOverlay').classList.remove('active')" style="flex:1;padding:.65rem;border:1.5px solid var(--border);border-radius:12px;background:transparent;color:var(--t2);font-family:Tajawal,sans-serif;font-weight:800;cursor:pointer">إلغاء</button>
            <button onclick="_saveBuyEmpBar('${barId}','${emp.replace(/'/g,"\\'")}',${eq705})" style="flex:2;padding:.65rem;border:none;border-radius:12px;background:linear-gradient(135deg,#16a34a,#15803d);color:#fff;font-family:Tajawal,sans-serif;font-weight:900;cursor:pointer">💾 شراء</button>
        </div>
    </div>`;
    window._buyBarPaidState=true;
    ov.classList.add('active');
    setTimeout(()=>{const e=document.getElementById('buyBarPrice');if(e)e.focus();},300);
};
window._buyBarSetPaid=()=>{
    const paid=window._buyBarPaidState;
    const p=document.getElementById('buyBarPaid'),u=document.getElementById('buyBarUnpaid');
    if(p){p.style.background=paid?'#16a34a':'transparent';p.style.color=paid?'#fff':'#16a34a';}
    if(u){u.style.background=!paid?'#dc2626':'transparent';u.style.color=!paid?'#fff':'#dc2626';}
};
window._buyBarPreview=(eq705)=>{
    const p=readNum('buyBarPrice');
    const el=document.getElementById('buyBarTotal');
    if(el)el.innerHTML=p>0?`المجموع: <b style="color:#16a34a">${fmtDin(p*eq705)} دج</b>`:'';
};
window._saveBuyEmpBar=(barId,emp,eq705)=>{
    const bar=(g730||[]).find(b=>b.id===barId&&b.empOwner===emp);
    if(!bar)return toast('السبيكة غير موجودة','error');
    const price=readNum('buyBarPrice');
...
window.openEmpStock=()=>{
    ...
    let ov=document.getElementById('empStockOverlay');
    if(!ov){
        ov=document.createElement('div');ov.id='empStockOverlay';ov.className='modal-overlay';
        document.body.appendChild(ov);
        ov.addEventListener('click',e=>{if(e.target===ov)ov.classList.remove('active');});
    }
    const totEq=myBars.reduce((s,b)=>s+(Number(b.w)||0)*((Number(b.k)||730)/705),0);
...
window.openEmpPay=()=>{
    let ov=document.getElementById('empPayOverlay');
    if(!ov){
        ov=document.createElement('div');ov.id='empPayOverlay';ov.className='modal-overlay';
        document.body.appendChild(ov);
        ov.addEventListener('click',e=>{if(e.target===ov)ov.classList.remove('active');});
    }
    ov.innerHTML=`<div class="modal" style="max-width:380px">
        <div class="modal-handle"></div>
        <h3 style="text-align:center;color:#16a34a">💵 دفع</h3>
        <div style="font-size:.68rem;color:var(--t3);text-align:center;margin-bottom:.6rem">المبلغ يدخل صوارد كوفرك ويظهر عند الإدارة</div>
        <input type="text" inputmode="decimal" id="empPayAmount" placeholder="💰 المبلغ (دج)" dir="ltr"
            style="width:100%;padding:.7rem;border-radius:10px;border:1.5px solid #16a34a;background:var(--card2);color:var(--t);font-family:Tajawal,sans-serif;font-size:1rem;font-weight:800;text-align:right;margin-bottom:.5rem;box-sizing:border-box"
            oninput="liveNum(this)">
        <input type="text" id="empPayNote" placeholder="📝 ملاحظة (اختياري)"
            style="width:100%;padding:.6rem;border-radius:10px;border:1.5px solid var(--border);background:var(--card2);color:var(--t);font-family:Tajawal,sans-serif;font-size:.85rem;text-align:right;margin-bottom:.6rem;box-sizing:border-box">
        <div style="display:flex;gap:.5rem">
            <button onclick="document.getElementById('empPayOverlay').classList.remove('active')"
                style="flex:1;padding:.65rem;border:1.5px solid var(--border);border-radius:12px;background:transparent;color:var(--t2);font-family:Tajawal,sans-serif;font-weight:800;cursor:pointer">إلغاء</button>
            <button onclick="saveEmpPay()"
                style="flex:2;padding:.65rem;border:none;border-radius:12px;background:linear-gradient(135deg,#16a34a,#15803d);color:#fff;font-family:Tajawal,sans-serif;font-weight:900;cursor:pointer">💾 حفظ</button>
        </div>
    </div>`;
    ov.classList.add('active');
    setTimeout(()=>{const e=document.getElementById('empPayAmount');if(e)e.focus();},300);
};
window.saveEmpPay=()=>{
    const a=readNum('empPayAmount');
    const note=(document.getElementById('empPayNote').value||'').trim();
    if(!a||a<=0)return toast('أدخل المبلغ','error');
    const nowStr=new Date().toLocaleDateString('fr-FR',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'});
    emitEvent('EMP_PAY',
        {emp:_currentUser,a,note},
        {op:{c:_currentUser,t:'دفع (موظف)',m:'دينار',a,_ts:Date.now(),dt:nowStr,note,empOwner:_currentUser}}
    );
    document.getElementById('empPayOverlay').classList.remove('active');
...
window.openExpense=()=>{
    ...
    document.getElementById('expKind').value='';
    const _isEmp=window._userRole==='employee';
    /* 👤 النوع يُضبط تلقائياً وتُخفى الأزرار للجميع:
       الموظف → قوبال · الأدمين → حانوت (لا داعي لاختيار «قوبال» عنده) */
    const _kindRow=document.getElementById('expKindShop')?.parentElement;
    if(_kindRow)_kindRow.style.display='none';
    _setExpKind(_isEmp?'قوبال':'حانوت');
...
/* اختيار نوع المصروف (حانوت/قوبال) — إلزامي */
window._setExpKind=(kind)=>{
    document.getElementById('expKind').value=kind||'';
    const shop=document.getElementById('expKindShop'), gob=document.getElementById('expKindGobal');
    if(shop){shop.style.background=kind==='حانوت'?'var(--g500)':'transparent';shop.style.color=kind==='حانوت'?'#fff':'var(--g600)';}
    if(gob){gob.style.background=kind==='قوبال'?'#7c3aed':'transparent';gob.style.color=kind==='قوبال'?'#fff':'#7c3aed';}
};
/* 🗂️ أرشيف المصاريف مع فلترة (حانوت / قوبال / الكل) */
window._expFilter='all';
window.openExpArchive=()=>{
    _expFilter='all';
    let ov=document.getElementById('expArchiveOverlay');
    if(!ov){
        ov=document.createElement('div');ov.id='expArchiveOverlay';ov.className='modal-overlay';
        document.body.appendChild(ov);
        ov.addEventListener('click',e=>{if(e.target===ov)ov.classList.remove('active');});
    }
    _renderExpArchive();
...
window._setExpFilter=(f)=>{ _expFilter=f; _renderExpArchive(); };
function _renderExpArchive(){
    const ov=document.getElementById('expArchiveOverlay'); if(!ov)return;
    /* 👤 أرشيف الأدمين = مصاريفه فقط · الموظف = مصاريفه (مصاريف الموظفين في لاقوبال) */
    const _me=window._currentUser, _isEmp=window._userRole==='employee';
    const all=ops.filter(o=>o.t==='مصاريف'&&(_isEmp?o.empOwner===_me:!o.empOwner)).sort((a,b)=>(b._ts||0)-(a._ts||0));
    const filtered=_expFilter==='all'?all:all.filter(o=>(o.expKind||'')===_expFilter);
    const tot=filtered.reduce((s,o)=>s+(o.a||0),0);
    const totShop=all.filter(o=>o.expKind==='حانوت').reduce((s,o)=>s+(o.a||0),0);
    const totGobal=all.filter(o=>o.expKind==='قوبال').reduce((s,o)=>s+(o.a||0),0);
    const _btn=(f,lbl,col)=>`<button onclick="_setExpFilter('${f}')" style="flex:1;padding:.5rem;border-radius:10px;border:1.5px solid ${col};background:${_expFilter===f?col:'transparent'};color:${_expFilter===f?'#fff':col};font-family:Tajawal,sans-serif;font-weight:900;font-size:.78rem;cursor:pointer">${lbl}</button>`;
    ov.innerHTML=`<div class="modal" style="max-width:460px">
        <div class="modal-handle"></div>
        <h3 style="text-align:center">🗂️ أرشيف المصاريف</h3>
        ${totGobal>0?`<div style="display:flex;gap:.4rem;margin:.5rem 0">
            ${_btn('all','الكل','#0369a1')}
            ${_btn('حانوت','حانوت','#16a34a')}
            ${_btn('قوبال','قوبال','#7c3aed')}
        </div>`:''}
        ${totGobal>0?`<div style="display:flex;gap:.4rem;margin-bottom:.6rem;font-size:.62rem;text-align:center">
            <div style="flex:1;background:rgba(22,163,74,.1);border-radius:8px;padding:.35rem"><b style="color:#16a34a">${fmtDin(totShop)}</b><br>حانوت</div>
            <div style="flex:1;background:rgba(124,58,237,.1);border-radius:8px;padding:.35rem"><b style="color:#7c3aed">${fmtDin(totGobal)}</b><br>قوبال</div>
        </div>`:''}
        <div style="max-height:50vh;overflow-y:auto">
            ${filtered.length?filtered.map(o=>{
                const kc=o.expKind==='حانوت'?'#16a34a':o.expKind==='قوبال'?'#7c3aed':'#6b7280';
                return`<div style="display:flex;justify-content:space-between;align-items:center;padding:.55rem .6rem;border-bottom:1px solid var(--border);gap:.5rem">
                    <div style="flex:1;min-width:0">
                        <div style="font-weight:800;font-size:.8rem;color:#dc2626">−${fmtDin(o.a||0)} دج</div>
                        <div style="font-size:.62rem;color:var(--t3)">${o.dt||''}${o.note?' · '+o.note:''}</div>
                    </div>
                    <span style="font-size:.6rem;font-weight:900;color:${kc};background:${kc}22;padding:.15rem .5rem;border-radius:1rem;white-space:nowrap">${o.expKind||'—'}</span>
                </div>`;}).join(''):'<div style="text-align:center;color:var(--t3);padding:2rem">لا توجد مصاريف في هذا القسم</div>'}
        </div>
        <div style="margin-top:.5rem;padding:.5rem;background:var(--card2);border-radius:10px;text-align:center;font-weight:900;font-size:.85rem">
            الإجمالي (${_expFilter==='all'?'الكل':_expFilter}): <span style="color:#dc2626">${fmtDin(tot)} دج</span>
        </div>
        <button onclick="document.getElementById('expArchiveOverlay').classList.remove('active');openExpense()"
            style="width:100%;margin-top:.5rem;padding:.7rem;border:none;border-radius:12px;background:linear-gradient(135deg,#dc2626,#b91c1c);color:#fff;font-family:Tajawal,sans-serif;font-weight:900;font-size:.9rem;cursor:pointer">＋ مصروف جديد</button>
        <button onclick="document.getElementById('expArchiveOverlay').classList.remove('active')"
            style="width:100%;margin-top:.5rem;padding:.6rem;border:none;border-radius:10px;background:rgba(120,120,120,.12);color:var(--t2);font-family:Tajawal,sans-serif;font-weight:800;cursor:pointer">إغلاق</button>
    </div>`;
}
window.saveExp=()=>{
    ...
    const _isEmp=window._userRole==='employee';
    const kind=_isEmp?'قوبال':'حانوت';  /* تلقائي: الموظف قوبال · الأدمين حانوت */
    const cur=document.querySelector('input[name="expCur"]:checked').value;
    if(!a||a<=0)return toast('حدد قيمة المصروف','error');
    if(!kind)return toast('⚠️ اختر نوع المصروف: حانوت أو قوبال','error');
    const _emp=_isEmp?_currentUser:null;
    const nowStr=new Date().toLocaleDateString('fr-FR',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'});
    if(cur==='دولار'){
        const cust=document.getElementById('expCustomer').value.trim();
        if(!cust)return toast('أدخل اسم الزبون للمصاريف بالسلعة','error');
        emitEvent('EXPENSE',{cur,a,n,cust,kind,empOwner:_emp},{op:{c:cust,t:'مصاريف',m:'دولار',a,_ts:Date.now(),dt:nowStr,note:n,expKind:kind,empOwner:_emp}});
        closeModal('expModal');
        toast(`💸 مصاريف ${a.toLocaleString('fr-FR')}$ — ${kind} — لصالح ${cust}`);
    }else{
        if(B.دينار<a-0.001)return toast('⚠️ السيولة غير كافية','error');
        emitEvent('EXPENSE',{cur,a,n,kind,empOwner:_emp},{op:{c:n||('مصاريف — '+kind),t:'مصاريف',m:'دينار',a,_ts:Date.now(),dt:nowStr,note:n,expKind:kind,empOwner:_emp}});
        closeModal('expModal');
        if(_isEmp&&window._renderEmpHome)setTimeout(_renderEmpHome,200);
        toast(`💸 تم خصم مصروف ${kind}`);
    }
};
...
function opDetailLines(o,custView){
    ...
    if(o.note && o.t!=='تذويب كاصي حي') lines.push(`📝 ${o.note}`);
    return lines;
}
...
window._logToday=()=>{
    ...
    const el=document.getElementById('logDayFilter'); if(el)el.value=iso;
    renderLog();
};
window._logClearDay=()=>{
    const el=document.getElementById('logDayFilter'); if(el)el.value='';
    renderLog();
};
/* 📅 فلترة السجل باليوم */
window._logToday=()=>{
...
function renderLog(){
    ...
    const day=document.getElementById('logDayFilter')?.value||'';
    let fl=ops;
    /* 👤 سجل الأدمين لا يخلط عمليات الموظفين (لها لاقوبال) — إلا للموظف نفسه يرى عملياته */
    if(window._userRole==='employee'){ fl=fl.filter(o=>o.empOwner===_currentUser); }
    else{ fl=fl.filter(o=>!o.empOwner); }
    if(s)fl=fl.filter(o=>(o.c||'').toLowerCase().includes(s));
    if(f!=='all')fl=fl.filter(o=>o.t===f);
    /* 📅 فلترة باليوم: قارن تاريخ العملية باليوم المختار */
    if(day){
        const dStart=new Date(day+'T00:00:00').getTime();
        const dEnd=dStart+86400000;
        fl=fl.filter(o=>{ const ts=o._ts||0; return ts>=dStart&&ts<dEnd; });
    }
    ...
    list.innerHTML=fl.map(o=>{
        ...
        return`<div class="log-item" style="align-items:flex-start">
            ...
            <span style="flex:1;min-width:0">
                ...
                <br><small style="color:var(--t2)">${o.dt||''} · <span style="color:${bg};font-weight:700">${_realOpName(o)}</span></small>${detailHtml}${byHtml}
            </span>
...
window.delOp=(id)=>{
    /* 🔒 حارس الملكية: كلٌّ يحذف عملياته فقط */
    const _op=ops.find(o=>String(o.id)===String(id));
    if(_op){
        const _me=window._currentUser, _isEmp=window._userRole==='employee';
        const mine=_isEmp?(_op.empOwner===_me):(!_op.empOwner);
        if(!mine)return toast('🚫 لا يمكنك حذف عملية ليست لك','error');
    }
...
function renderDebts(){
    ...
    const _adminDebts=debts.filter(d=>!d.empOwner);
    if(!_adminDebts.length){tb.innerHTML='<tr><td colspan="4" style="padding:2rem;color:var(--t3)">لا توجد ديون</td></tr>';if(tf)tf.innerHTML='';return}
    const cd={};
    debts.forEach(d=>{
        /* 👤 ديون الموظفين لا تُجمع مع ديون الأدمين — تظهر في بطاقة لاقوبال فقط */
        if(d.empOwner)return;
    ...
    tb.innerHTML=entries
        ...
        .map(([n,v])=>{
            /* 👤 زبون الموظف يُوسم «قوبال» */
            const _empCust={};
            (debts||[]).forEach(d=>{ if(d.empOwner)_empCust[d.c]=true; });
            const tag=_empCust[n]
                ?'<span style="font-size:.55rem;background:rgba(124,58,237,.15);color:#7c3aed;padding:.05rem .3rem;border-radius:.5rem;margin-right:.2rem">قوبال</span>'
                :((kind[n]||'market')==='workshop'?'<span style="font-size:.55rem;background:rgba(194,65,12,.15);color:#c2410c;padding:.05rem .3rem;border-radius:.5rem;margin-right:.2rem">ورشة</span>':'<span style="font-size:.55rem;background:rgba(3,105,161,.15);color:#0369a1;padding:.05rem .3rem;border-radius:.5rem;margin-right:.2rem">سوق</span>');
            return`<tr>
...
window.doXfer=()=>{
    ...
    const nowStr=new Date().toLocaleDateString('fr-FR',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'});
    const _xEmp=(window._userRole==='employee'?_currentUser:null);
    emitEvent('XFER',
        {from:_settleCustomer,to,srcType:_xferSrcType,dstType,srcDelta:sign*W,dstDelta:sign*wDst,w:W,wDst,empOwner:_xEmp},
        {op:{c:_settleCustomer,t:'تحويل لزبون',m:_xferSrcType,a:W,_ts:Date.now(),dt:nowStr,empOwner:_xEmp||undefined,
...
function _cashSettlePrompt(type){
    ...
    emitEvent('SETTLE_CASH',
        {empOwner:(window._userRole==='employee'?_currentUser:undefined),c:_settleCustomer,type,net,amt},
        {op:{c:_settleCustomer,t:'تصفية',m:type,a:net>0?amt:-amt,_ts:Date.now(),dt:nowStr,partial:amt<Math.abs(net)-0.001,empOwner:(window._userRole==='employee'?_currentUser:undefined)}}
    );
    return true;
}
function _applySettle(type){
    ...
    const nowStr=new Date().toLocaleDateString('fr-FR',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'});
    emitEvent('SETTLE',
        {empOwner:(window._userRole==='employee'?_currentUser:undefined),c:_settleCustomer,type,net,barsRemove,barUpdates},
        {op:{c:_settleCustomer,t:'تصفية',m:type,a:net,_ts:Date.now(),dt:nowStr,empOwner:(window._userRole==='employee'?_currentUser:undefined)}}
    );
    return true;
}
...
window._confirm730With24=function(){
    ...
    emitEvent('SETTLE_730_24',
        ...
        {op:{c:_settleCustomer,t:'تصفية',m:'ذهب 730',a:net>0?partial:-partial,_ts:Date.now(),dt:nowStr,crossKarat:true,paid24:equiv24,partial:remaining>0.001,empOwner:(window._userRole==='employee'?_currentUser:undefined)}}
    );
...
window._confirm24FromInv=function(){
    ...
    emitEvent('SETTLE_24_INV',
        {empOwner:(window._userRole==='employee'?_currentUser:undefined),c:_settleCustomer,partial,net,remaining,isGiving,barsRemove,barUpdates,barsAdd},
        {bars:Object.keys(dispBars).length?dispBars:undefined,
...
function _renderArchiveChips(){
    ...
    /* 👤 الموظف: بيع وشراء فقط */
    const _isEmp=window._userRole==='employee';
    const chips=_isEmp
        ? [['all','📋 الكل'],['buy','🟢 شراء'],['sell','🔴 بيع']]
        : [['all','📋 الكل'],['doll','🛍️ سلعة'],['buy','🟢 شراء'],['sell','🔴 بيع'],['raf','🔥 رافيناج']];
...
function renderArchive(){
    ...
    /* 👤 عزل: الأدمين يرى فواتيره · الموظف يرى فواتيره (فواتير الموظفين لها لاقوبال) */
    const _me=window._currentUser, _isEmp=window._userRole==='employee';
    const _mineInv=(v)=> _isEmp ? (v&&v.empOwner===_me) : (!(v&&v.empOwner));
    _renderArchiveChips();
    const f=_archiveFilter;
    const _sec=(id,vis)=>{const e=document.getElementById(id);if(e)e.style.display=vis?'':'none';};
    /* 👤 الموظف: أرشيف فواتير البيع والشراء فقط (بلا رافيناج ولا سلعة) */
    _sec('archSec-gold', f==='all'||f==='buy'||f==='sell');
    _sec('archSec-raf',  !_isEmp&&(f==='all'||f==='raf'));
    _sec('archSec-doll', !_isEmp&&(f==='all'||f==='doll'));
    /* فواتير الشراء/البيع */
    const _invMine=invoices.filter(_mineInv);
    const goldList=f==='buy'?_invMine.filter(i=>i.t==='buy'):f==='sell'?_invMine.filter(i=>i.t==='sell'):_invMine;
    ...
    /* أرشيف الرافيناج */
    const _rafArch=rafInvoices.filter(_mineInv);
    document.getElementById('rafArchiveCount').textContent=_rafArch.length;
    document.getElementById('rafArchiveList').innerHTML=_rafArch.length?_rafArch.map(r=>`
        <div class="saved-card">
            <div>
                <strong>${r.c}</strong>
    ...
    /* أرشيف الدولار */
    const _dollArch=dollInvoices.filter(_mineInv);
    document.getElementById('dollArchiveCount').textContent=_dollArch.length;
    document.getElementById('dollArchiveList').innerHTML=_dollArch.length?_dollArch.map(d=>`
        <div class="saved-card">
            <div>
                <strong>👤 ${d.c}</strong>
...
/* ═══════════ NAVIGATION ═══════════ */
/* 🔐 تطبيق صلاحيات الدور: الموظف يرى فاتورة بيع/شراء ومصاريف فقط */
window._applyRolePerms=()=>{
    ...
    if(!isEmp){
        /* 🔓 الأدمين: أعِد إظهار كل ما قد يكون أُخفي في جلسة موظف سابقة على الجهاز */
        ['nav-raffinage','goldSaleCard','archSec-raf','archSec-doll','adminOnlySettings'].forEach(id=>{
            const el=document.getElementById(id); if(el&&el.style.display==='none')el.style.display='';
        });
        return;
    }
    /* 🔒 الموظف: إخفاء صريح بـ JS (لا نعتمد على CSS وحده) */
    ['nav-raffinage','goldSaleCard','archSec-raf','archSec-doll','adminOnlySettings'].forEach(id=>{
        const el=document.getElementById(id); if(el)el.style.display='none';
    });
    /* زر «الرئيسية» يفتح صفحة الموظف · زر «الديون» يفتح دفتر ديونه */
    const _nh=document.getElementById('nav-home');
    if(_nh)_nh.setAttribute('onclick','_showEmpHome()');
    const _nd=document.getElementById('nav-debts');
    if(_nd){ _nd.style.display=''; _nd.setAttribute('onclick','_showEmpDebts()'); }
    /* اعرض صفحة الموظف فوراً (بلا تأخير يسبّب ومضة) */
    try{ _showEmpHome(); }catch(e){ setTimeout(()=>{try{_showEmpHome();}catch(_){}} ,60); }
};
/* 👤 عرض صفحة الموظف وتحديث بياناتها */
window._showEmpHome=()=>{
    document.querySelectorAll('.page').forEach(x=>x.classList.remove('active'));
    document.querySelectorAll('.ni').forEach(x=>x.classList.remove('active'));
    const p=document.getElementById('page-empHome'); if(p)p.classList.add('active');
    const nh=document.getElementById('nav-home'); if(nh)nh.classList.add('active');
    _renderEmpHome();
};
/* 👤 عرض صفحة ديون الموظف (مستقلة) */
window._showEmpDebts=()=>{
    document.querySelectorAll('.page').forEach(x=>x.classList.remove('active'));
    document.querySelectorAll('.ni').forEach(x=>x.classList.remove('active'));
    const p=document.getElementById('page-empDebts'); if(p)p.classList.add('active');
    const nd=document.getElementById('nav-debts'); if(nd)nd.classList.add('active');
    _renderEmpHome();
};
window._renderEmpHome=()=>{
    if(window._userRole!=='employee')return;
    const me=_currentUser;
    /* صوارد الكوفر = سيولة الموظف (تُجمع من دفعاته ناقص مصاريفه) */
    const coffer=(window._empCoffer&&window._empCoffer[me])||0;
    ...
    const cd={};
    debts.forEach(d=>{ if(d.empOwner===me){ if(!cd[d.c])cd[d.c]={di:0,do:0,g7:0,g2:0}; cd[d.c][d.type==='دينار'?'di':d.type==='دولار'?'do':d.type==='ذهب 730'?'g7':'g2']+=(d.a||0); } });
    const entries=Object.entries(cd).filter(([,v])=>Math.abs(v.di)>0.001||Math.abs(v.do)>0.001||Math.abs(v.g2)>0.001);
    const lastTx={}; ops.forEach(o=>{ if(o&&o.c){ const t=o._ts||0; if(t>(lastTx[o.c]||0))lastTx[o.c]=t; } });
    const fD=(v,d=0,unit='')=>{
        if(!v||Math.abs(v)<0.001)return'—';
        const isDin=unit==='Da'||unit==='دج';
        const shown=isDin?fmtDin(v):fmt(v,d);
    ...
    const tb=document.getElementById('empDebtsBody');
    const tf=document.getElementById('empDebtsFoot');
    if(tb){
        if(!entries.length){ tb.innerHTML='<tr><td colspan="4" style="padding:2rem;color:var(--t3)">لا يوجد زبائن بعد</td></tr>'; if(tf)tf.innerHTML=''; }
        else{
            tb.innerHTML=entries
                .sort((a,b)=>{ const ta=lastTx[a[0]]||0,tb2=lastTx[b[0]]||0; if(tb2!==ta)return tb2-ta; return Math.abs(b[1].di)-Math.abs(a[1].di); })
                .map(([n,v])=>`<tr>
                    <td><strong style="cursor:pointer;color:var(--g600);text-decoration:underline;text-decoration-style:dotted" onclick="openGoodsFor('${n.replace(/'/g,"\\'")}')" title="فتح بطاقة الزبون">${n}</strong></td>
                    <td>${fD(v.di,0,'Da')}</td><td>${fD(v.do,2,'غ')}</td>
                    <td>${fD(v.g2,2,'غ (24)')}</td>
                </tr>`).join('');
            const tot=entries.reduce((s,[,v])=>({di:s.di+v.di,do:s.do+v.do,g2:s.g2+v.g2}),{di:0,do:0,g2:0});
...
window.switchPage=(p)=>{
    /* 🔐 الموظف: فاتورة فقط */
    if(window._userRole==='employee'&&!['invoice','empHome','log','archive'].includes(p)){
        /* زر الرئيسية للموظف → صفحته الخاصة */
        if(p==='home'){ try{_showEmpHome();}catch(e){} return; }
        if(p==='debts'){ try{_showEmpDebts();}catch(e){} return; }
        return toast('غير مسموح — صلاحيتك: الرئيسية والفاتورة والسجل والأرشيف','error');
    }
    const er=window._editRestore;
...
(function(){
    ...
    function _snapName(inp){
        /* 🚫 أُلغي التصحيح التلقائي: كان يبدّل أسماء صحيحة («احمد»→«محمد»).
           الاقتراح يبقى في القائمة المنسدلة، لكن لا نغيّر ما يكتبه المستخدم. */
        return;
    }
...
(function _arrowNav(){
    ...
    document.addEventListener('keydown',e=>{
        ...
        /* 🔓 استثنِ حقول الدخول وتغيير كلمة المرور من التنقل بالأسهم */
        if(el.closest('#loginMainPanel, #loginOverlay, .login-box'))return;
