...
window.onInvTypeChange=function(){
    ...
    /* 🔒 كل مستخدم يرى سبائكه للبيع: الأدمين سبائكه (بلا الموظفين) · الموظف سبائكه */
    const _me=window._currentUser, _isEmp=window._userRole==='employee';
    const _mine=(b)=> _isEmp ? (b.empOwner===_me) : (!b.empOwner);
    const all=[...g24.filter(_mine).map(b=>({...b,btype:'24'})),...g730.filter(_mine).map(b=>({...b,btype:'730'}))];
...
window.saveInvoice=()=>{
    ...
    if(t==='sell'){
        ...
        /* 🔒 المتاح = سبائك المستخدم الحالي فقط (الأدمين بلا سبائك الموظفين) */
        const _me=window._currentUser, _isEmp=window._userRole==='employee';
        const _mine=(b)=> _isEmp ? (b.empOwner===_me) : (!b.empOwner);
        const availByK={};
        g730.filter(_mine).forEach(b=>{const key=_kKey('730',b.k||730);availByK[key]=(availByK[key]||0)+(b.w||0);});
        g24 .filter(_mine).forEach(b=>{const key=_kKey('24', b.k||1000);availByK[key]=(availByK[key]||0)+(b.w||0);});
    ...
    if(t==='buy'){
        ...
        function pickAndRemove(pool,w,k){
            ...
            /* 🔒 الأدمين لا يبيع سبائك موظفيه · الموظف لا يبيع إلا سبائكه هو */
            const me=window._currentUser;
            const isEmp=window._userRole==='employee';
            const _mine=(b)=> isEmp ? (b.empOwner===me) : (!b.empOwner);
            let rem=w;
            for(let i=bars.length-1;i>=0&&rem>0.001;i--){
                if(k!=null&&!same(bars[i]))continue;
                if(!_mine(bars[i]))continue;   /* تخطَّ سبائك غير المالك */
            ...
            /* لم تكفِ سبائك المالك؟ نبّه بدل بيع سبائك غيره */
            if(rem>0.001){
                const _av=bars.filter(b=>(k==null||same(b))&&_mine(b)).reduce((s,b)=>s+b.w,0);
                throw new Error(`مخزون 705 غير كافٍ من سبائكك — المتاح ${_av.toFixed(2)}غ${isEmp?'':' (سبائك الموظفين لا تُباع من حسابك)'}`);
            }
        }
        try{
        newItems.forEach(item=>{
            if(item.sbid){
                ...
                if(ex){
                    /* 🔒 منع بيع سبيكة ليست ملك المستخدم الحالي */
                    const me=window._currentUser, isEmp=window._userRole==='employee';
                    const mine=isEmp?(ex.empOwner===me):(!ex.empOwner);
                    if(!mine)throw new Error(isEmp?'لا يمكنك بيع سبيكة ليست في مخزونك':'هذه السبيكة ملك موظف — لا تُباع من حسابك');
        ...
        }catch(err){ return toast('⚠️ '+(err&&err.message||err),'error'); }
    }
    ...
    emitEvent(evType,
        {c,iid,tp,akhd,prevBal,barsAdd,barsRemove,barUpdates,empOwner:_emp},
        {
            invoice:_inv,
            bars:Object.keys(dispBars).length?dispBars:undefined,
            op:{c,t:t==='buy'?'شراء':'بيع',m:'دينار',a:tp,_ts:Date.now(),dt:nowStr,iid,empOwner:_emp}
        }
    );
    resetInvForm();
...
window.delInv=(id)=>{
    /* 🔒 حارس الملكية: كلٌّ يحذف فواتيره فقط */
    const _v=invoices.find(x=>x.id===id);
    const _me=window._currentUser, _isEmp=window._userRole==='employee';
    if(_v){
        const mine=_isEmp?(_v.empOwner===_me):(!_v.empOwner);
        if(!mine)return toast('🚫 لا يمكنك حذف فاتورة ليست لك','error');
    }
...
window.editInv=(id)=>{
    /* 🔒 حارس الملكية: كلٌّ يعدّل فواتيره فقط */
    const _ev=invoices.find(x=>x.id===id);
    if(_ev){
        const _me=window._currentUser, _isEmp=window._userRole==='employee';
        const mine=_isEmp?(_ev.empOwner===_me):(!_ev.empOwner);
        if(!mine)return toast('🚫 لا يمكنك تعديل فاتورة ليست لك','error');
    }
